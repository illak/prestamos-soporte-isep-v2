...
export default function UsuarioImport({ isOpen, onClose, onSuccess }) {
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      const response = await usuariosApi.import(file, modoConflicto);
      setResult(response);
      if (response.resumen.procesados > 0) {
        toast.success(`Se importaron ${response.resumen.procesados} usuario(s)`);
      }
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Importar Usuarios" size="lg">
      <div className="space-y-4">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <p className="text-sm text-blue-600 mt-2">
            El campo rol debe ser "usuario" o "soporte_it"
          </p>
        </div>
        ...
        {/* Modo conflicto */}
        <div>
          <label className="label">Manejo de duplicados</label>
          <select
            value={modoConflicto}
